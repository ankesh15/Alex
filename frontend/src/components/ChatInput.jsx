...
import { ArrowUp, Loader2, Globe, Library, FileText, ChevronDown } from 'lucide-react';
...
export default function ChatInput({
  ...
  const getPlaceholder = () => {
    ...
    if (selectedScope === 'all') {
      return readyDocs.length === 0
        ? 'Upload documents to search across them, or switch to General...'
  ...
  return (
    <div className="w-full max-w-3xl mx-auto px-4 pb-4 sm:pb-6">
      ...
      <div className="relative bg-white border border-slate-300 hover:border-slate-400 focus-within:border-slate-500 focus-within:ring-2 focus-within:ring-slate-100 rounded-2xl shadow-sm transition-all p-3">
        {/* Textarea */}
        <textarea
          ref={textareaRef}
          value={text}
        ...
        <div className="flex items-center justify-between pt-2 mt-1 border-t border-slate-100">
          ...
          <div className="relative" ref={dropdownRef}>
            <button
              ...
              className="inline-flex items-center space-x-1.5 text-xs font-medium text-slate-700 hover:text-slate-900 bg-slate-100 hover:bg-slate-200/80 px-2.5 py-1.5 rounded-lg transition-colors"
            >
            ...
            {dropdownOpen && (
              <div className="absolute bottom-full mb-2 left-0 w-64 bg-white border border-slate-200 rounded-xl shadow-lg py-1.5 z-50 text-xs">
                ...
                >
                  ...
                  <div>
                    <div className="font-medium">All Documents ({readyDocs.length})</div>
                    <div className="text-[10px] text-slate-400">RAG search across all indexed documents</div>
                  </div>
                </button>
      ...
      <div className="text-[11px] text-center text-slate-400 mt-2">
        Alex answers general queries and queries uploaded documents using grounded RAG. Verify citations for critical research.
      </div>
    </div>
  );
}
